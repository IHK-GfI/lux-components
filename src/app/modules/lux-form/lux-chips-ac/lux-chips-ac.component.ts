...
export class LuxChipsAcComponent extends LuxFormComponentBase implements AfterViewInit, OnDestroy {
  ...
  @ContentChildren(LuxChipAcGroupComponent) luxChipAcGroupComponents: QueryList<LuxChipAcGroupComponent>;
  ...
  @Input() set luxDisabled(disabled: boolean) {
    ...
    setTimeout(() => {
      this.luxChipAcGroupComponents.forEach((chipGroup) => (chipGroup.luxDisabled = disabled));
      this.luxChipAcComponents.forEach((chip) => (chip.luxDisabled = disabled));
    });
  }
